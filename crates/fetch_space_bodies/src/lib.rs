...
async fn get_text_from_url_async(client: Client, url: String) -> Option<String> {
...
pub async fn get_body_motion(client: Client, id: i64) -> Option<(Position, Velocity)> {
    ...
    let text = get_text_from_url_async(client, url).await?;
...
pub async fn search_bodies(
    ...
    let response_buf = get_text_from_url_async(client, url).await?;
...
pub async fn get_body_properties(client: Client, id: i64) -> Option<(Mass, f64)> {
    ...
    let response_buf: String = get_text_from_url_async(client, url).await?;
